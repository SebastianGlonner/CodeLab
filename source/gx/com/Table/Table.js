...
gx.com.Table = new Class({
	...
	build: function () {
		...
		try {
			...
			this._display.divMessage = new Element('div', {
				'html' : this.options.noItemsText,
				'class': 'empty_box',
			});

			if ( this.options.showDefaultEmptyText )
				this._display.root.adopt(this._display.divMessage);
	...
	addData: function (data) {
		...
		try {
			...
			if ( data.length === 0 && this._anyData === false ) {
				this._display.divMessage.show();
			} else {
				this._anyData = true;
			}
